...
# This creates the metrics matcher
...
resource "aws_api_gateway_integration" "post_metrics" {
  rest_api_id = aws_api_gateway_rest_api.this.id
  resource_id = aws_api_gateway_method.post_metrics.resource_id
  http_method = aws_api_gateway_method.post_metrics.http_method

  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = data.aws_lambda_function.post_metrics.invoke_arn
}
...
resource "aws_api_gateway_deployment" "this" {
  ...
  depends_on = [
    aws_api_gateway_integration.post_metrics,
  ...
  triggers = {
    post_metrics_arn = data.aws_lambda_function.post_metrics.invoke_arn
    ...
    redeployment = sha1(join(",", list(
      ...
      jsonencode(aws_api_gateway_integration.post_metrics),
    )))
  }
